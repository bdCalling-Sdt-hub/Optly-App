...
//
//   DatePeriod next() {
//     return DatePeriod(startDate.add(const Duration(days: 7)));
//   }
//
//   DatePeriod previous() {
...
class DatePeriod {
  ...
  DatePeriod(DateTime focusDate)
      : startDate = focusDate.subtract(Duration(days: focusDate.weekday - 1)),
        endDate = focusDate.subtract(Duration(days: focusDate.weekday - 1)).add(Duration(days: 6));
  ...
  List<Map<String, dynamic>> getSortDayNameAndDates() {
    ...
    for (int i = 0; i < 7; i++) {
      DateTime currentDate = startDate.add(Duration(days: i));
      days.add({
        'day': DateFormat('EEE', 'de').format(currentDate), // Short form day name in German
        'date': currentDate, // DateTime object
      });
    }
    return days;
